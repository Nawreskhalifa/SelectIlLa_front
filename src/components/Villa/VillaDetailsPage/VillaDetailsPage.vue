 <template>
<div v-if="villa && villa.attributes" >
          <BreadCrumb :PrevPage="'villas list'" :url="'/villalist'" :PageTitle="villa.attributes?.name" />
    <div
    ...
    <div class="card-body">
      <div class="row "  style="display:flex ; align-items:'end' !important ; ">
        <div class="col-lg-5 col-md-9">
          ...
          >
<swiper-slide v-for="product in villa?.attributes?.photos?.data" :key="product.id">
              <img :src="getFullImageUrl(product.attributes?.url)" alt="product-details" />
            </swiper-slide>
          </swiper>
        </div>
        <div class="col-lg-2 col-md-3" v-if="villa && villa.attributes && villa.attributes?.photos  && villa.attributes.photos.data">
          <swiper
            :direction="'vertical'"
            :loop="true"
            :autoplay="{
...
<div class="product-details-content">
              ...
              <span class="text-success" v-if="villa && villa.attributes &&  villa.attributes?.isActive === false  ">
                <i
                  class="flaticon-eye lh-1 fs-16 position-relative top-1 me-1"
                ></i>
                ACTIVE
              </span>
                   <span class="text-danger" v-if="villa && villa.attributes &&  villa.attributes?.isActive === true  ">
                <i
...
<div class="content pe-0" v-if="villa && villa.attributes && villa.attributes.partner && villa.attributes.partner.data && villa.attributes.partner.data.attributes && villa.attributes.partner.data.attributes.user && villa.attributes.partner.data.attributes.user.data && villa.attributes.partner.data.attributes.user.data.attributes">
    ...
    <p class="fs-md-15 mb-0 text-paragraph lh-base"> Address : {{villa.attributes.partner.data.attributes.address}}</p>
 <span class="d-block text-black fs-md-15 fs-lg-16 fw-medium mb-8"></span>
    <p class="fs-md-15 mb-0 text-paragraph lh-base"> Phone  : {{villa.attributes.partner.data.attributes.phone}}</p>
...
<div class="row info-card">
    ...
    <div class="col-6 col-sm-4 col-xxl-6 col-xxxl-4" v-if="villa && villa.attributes && villa.attributes.partner && villa.attributes.partner.data && villa.attributes.partner.data.attributes && villa.attributes.partner.data.attributes.events">
        <div class="info p-10 p-sm-15 p-md-20">
            <span class="d-block text-black-emphasis mb-1">Events</span>
            <h4 class="d-block text-success fw-black mb-0">{{ villa.attributes.partner.data.attributes.events.data.length }}</h4>
        </div>
    </div>
    <div class="col-6 col-sm-4 col-xxl-6 col-xxxl-4" v-if="villa && villa.attributes && villa.attributes.partner && villa.attributes.partner.data && villa.attributes.partner.data.attributes && villa.attributes.partner.data.attributes.villas">
        <div class="info p-10 p-sm-15 p-md-20">
...
import { fetchVillaById } from "@/services/apiService";
 ...
 Autoplay,
  ...
  methods: {
    ...
    async fetchVilla() {
      ...
      const id = this.$route.params.id;
      const { data } = await fetchVillaById(id);
      this.villa = data;
