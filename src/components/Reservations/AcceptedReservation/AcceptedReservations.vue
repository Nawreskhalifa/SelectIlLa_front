<template>
      ...
      <div class="card-body p-15 p-sm-20 p-md-25">
        ...
        <div class="table-responsive">
          <table class="table text-nowrap align-middle mb-0">
            ...
            <tbody>
              <tr v-for="reservation in paginatedReservations" :key="reservation.id" @click="openModal(reservation)">
                <td class="shadow-none lh-1 fw-medium text-paragraph">
                  {{ reservation.attributes.Price }}
                </td>
                <td class="shadow-none lh-1 fw-medium text-paragraph">
                  {{ reservation.attributes.createdAt }}
                </td>
                <td class="shadow-none lh-1 fw-medium text-paragraph">
                  {{ reservation.attributes.customer.data.attributes.name }}
                </td>
                <td class="shadow-none lh-1 fw-medium text-paragraph">
                  {{ reservation.attributes.customer.data.attributes.phone }}
                </td>
                <td class="shadow-none lh-1 fw-medium text-paragraph">
                  <button @click="deleteReservation(reservation.id)" class=" bg-white  rounded-circle "  style="border: none;" ><i class="fa fa-trash" aria-hidden="true"></i>
                  </button>
                </td>
              </tr>
            </tbody>
         ...
         <pagination :total="totalPages" v-model="currentPage" @input="fetchReservationsData" />
      </div>
  ...
  <script>
  import { fetchAcceptedReservations, deleteAcceptedReservation } from "@/services/apiService";
    ...
    computed: {
      filteredReservations() {
        return this.reservations.filter(reservation => {
          const name = reservation.attributes.customer.data.attributes.name.toLowerCase();
          const phone = reservation.attributes.customer.data.attributes.phone.toLowerCase();
    ...
    methods: {
      ...
      async deleteReservation(id) {
        try {
          ...
          console.error(`Error deleting accepted reservation with ID ${id}:`, error);
        }
      },
      change() {
        this.currentPage = 1;
